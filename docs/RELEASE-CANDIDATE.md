...
Complete every unchecked row before publishing `3.0.0-rc.1` for opt-in testing.
...
- [x] Run Manual+, Training, Custom, and Relaxed automation independently and
  simultaneously. Confirm input, HUD, preview, config, rod, inventory, and result stay
  with the owning player.
- [x] Apply visibly different profiles, reconnect both players, reload the save, and
  confirm neither player's Apply action or profile changes the other.
- [x] Verify bait/tackle attachment, treasure collection, automatic eating, junk
  disposal, starter rod, and catch modifiers never change another player's inventory or
  result.
- [x] Connect a remote player while temporary enchantments are active. Confirm they are
  removed, remain unavailable while connected, and are never written into the save.
- [x] Cover join, disconnect, reconnect, host save, day end, warp, tool swap, and return
  to title during idle, cast, minigame, catch-popup, and treasure-menu states.
- [x] Inspect the SMAPI log after each topology for errors, repeated recovery, duplicated
  input, stale per-screen state, item loss, or duplication.

## 2. Festival and high-risk gameplay

- [x] On a disposable host test save, use Debug > Prepare Ice Fishing Festival or
  ...
  game.
- [x] With normal and golden treasure chance set to `Always`, test treasure targeting,
...
- [x] Test `SkipAll` and `SkipOnlyCaught` with first catches, legendary fish, fish ponds,
...
- [x] Test catch modifiers separately and together: difficulty, instant bite, perfect
  ...
  festival fishing.
- [x] Test Infinite Bait/Tackle and all temporary enchantments across catch, escape,
...
- [x] Use `fa_bubble` to verify Bubble Steering for manual and automatic casts at several
  cast powers. It must not move the player, extend cast range, or select a different
  power.
- [x] Run the complete cast-to-catch loop repeatedly with every automation stage toggled
  independently. Confirm no stuck input, reel audio, rod state, or uncontrolled recast.

## 3. Release readiness

- [x] Freeze the schema and review the Fishing Assistant 2 migration, future-schema
  read-only handling, and upgrade from both Fishing Assistant 2 and the latest beta.
- [x] Review English menu text, inline warnings, command help, README, configuration
  reference, compatibility notes, and limitations. Review Thai coverage separately.
- [x] Validate manifest version, unique ID, minimum SMAPI version, Nexus update key, and
  clean-install behavior.
- [x] Enable deterministic release ZIP packaging containing only the DLL, resolved
  manifest, translations, and required assets.
- [x] Test copied-save uninstall and reinstall, then add CI for restore, tests, Release
  build, and package inspection without proprietary game files.

## RC promotion

- [x] The full automated suite and a Release build pass with zero warnings.
- [x] No open issue can cause a crash, save corruption, item loss/duplication,
  cross-player control, or unrecoverable automation state.
- [x] Publish the inspected `3.0.0-rc.1` artifact for opt-in testing.
