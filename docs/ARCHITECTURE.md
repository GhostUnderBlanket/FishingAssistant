...
its full Vanilla color in every automation state; only the toolbar's own fade affects
its opacity. Animated Vanilla emotes above the rod distinguish active work (music note),
ordinary disable (X), menu pause, late-night stop (sleep), low-energy stop (sad), action
timeout (exclamation), and state-machine recovery (happy). An enabled idle session has
no emote. The badge decision consumes the owning session's state and `LastReason`
through a pure policy. Treasure targeting is intentionally config-only and has no
